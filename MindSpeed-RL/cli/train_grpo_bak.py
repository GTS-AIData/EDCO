...
# from mindspeed_rl.evaluation.grpo_eval import EvalActor
...
def train(config):
    ...
    if rl_config.rule_reward:
        ...
        for i in range(rule_reward_num_process):
            ...
            rule_reward.initialize.remote(reward_config, rl_config, tokenizer, generate_config.trust_remote_code)
...
def eval_model_provider(pre_process=True, post_process=True):
    """Builds the model.
    ...
    logger.info('building eval GPT model ...')
    config = core_transformer_config_from_args(args)
...
@hydra.main(config_path='../configs', config_name='grpo_qwen3_4b_A3', version_base=None)
def main(config):
