...
export task_name="$1"
export MASTER_ADDR="${2:-10.137.6.176}"
export pretrain_ckpt="$3"
export date="1021"
...
export CKPT_LOAD_DIR="/home/Qwen3/model_weight/${pretrain_ckpt}/"
...
echo "$(date): 开始迭代训练流程，共 $TOTAL_ROUNDS 轮" | tee -a "$LOG_FILE"


# ========= 初始化模型路径 =========
## 当前用于数据筛选的模型（HF 格式）
#CURRENT_HF_MODEL_PATH="$INITIAL_HF_MODEL_PATH"
## 当前用于微调的模型（MCore 格式）
...
if [ "$START_ROUND" -eq 1 ]; then
    ...
    CURRENT_HF_MODEL_PATH="${MODEL_OUTPUT_ROOT}/round${PREV_ROUND}_tuned_hf"
    CURRENT_MC_MODEL_PATH="${MODEL_OUTPUT_ROOT}/round${PREV_ROUND}_tuned_mc"
    if [ ! -d "$CURRENT_HF_MODEL_PATH" ] || [ ! -d "$CURRENT_MC_MODEL_PATH" ]; then
        echo "错误：第 ${PREV_ROUND} 轮模型不存在！"
        exit 1
    fi
fi
...
for round in $(seq $START_ROUND $TOTAL_ROUNDS); do
    ...
    # ========= 步骤1: 基于loss的数据筛选（使用 HF 模型） =========
    ray stop --force
    cd "$LLM_DIR" || { echo "$(date): 错误：无法进入工作目录: $LLM_DIR"; exit 1; }
    echo "$(date): [Round $round] 步骤1: 执行基于loss的数据筛选..." | tee -a "$LOG_FILE"
    STEP_START=$(date +%s)

    python data_selection_by_loss.py \
    ...
    python ./cli/generate_temp_config.py \
      configs/datasets/deepscaler_course_loop.yaml \
    ...
    TRAIN_CONFIG_NAME=grpo_qwen3_4B_A3_${task_name}_round${round}
    TRAIN_CONFIG_YAML="${TRAIN_CONFIG_DIR}/${TRAIN_CONFIG_NAME}.yaml"

    python ./cli/generate_temp_config.py \
      configs/grpo_qwen3_4b_A3_course_loop.yaml \
      "$TRAIN_CONFIG_YAML"
    ...
    bash ./examples/grpo/grpo_trainer_qwen3_4b_loop_coures.sh "${TRAIN_CONFIG_NAME}" "${MASTER_ADDR}"
    ...
    cd "$LLM_DIR" || { echo "$(date): 错误：无法进入工作目录: $LLM_DIR"; exit 1; }
    conda run -n mindspeed_llm_v1 bash "$LLM_DIR/examples/zyf/4B/ckpt_convert_qwen3_mcore2hf_loop.sh" \
        "$ROUND_MC_DIR" \
        "$ROUND_HF_DIR"
...
echo "$(date): 所有 $TOTAL_ROUNDS 轮训练完成！"
