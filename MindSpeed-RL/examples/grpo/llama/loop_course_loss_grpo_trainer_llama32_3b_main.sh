...
for round in $(seq $START_ROUND $TOTAL_ROUNDS); do
    ...
    python ./cli/generate_temp_config.py \
      configs/datasets/deepscaler_course_loop_llama.yaml \
    ...
    TRAIN_CONFIG_NAME=grpo_llama32_3b_A3_${task_name}_round${round}
    TRAIN_CONFIG_YAML="${TRAIN_CONFIG_DIR}/${TRAIN_CONFIG_NAME}.yaml"

    python ./cli/generate_temp_config.py \
      configs/grpo_llama32_3b_A3_course_loop.yaml \
      "$TRAIN_CONFIG_YAML"
    ...
    bash ./examples/grpo/llama/grpo_trainer_llama32_3b_loop_coures.sh "${TRAIN_CONFIG_NAME}" "${MASTER_ADDR}"
