...
def remove_boxed(s: str) -> str:
    ...
    left = "\\boxed{"
    if s[:len(left)] != left:
        raise ValueError(f"box error: 前缀不匹配，实际为 '{s[:len(left)]}'，完整内容: {s}")
    if s[-1] != "}":
        raise ValueError(f"box error: 结尾字符不匹配，实际为 '{s[-1]}'，完整内容: {s}")
...
def is_correct_strict_box(pred: str,
    ...
    if pause_tokens_index is not None:
        if len(pause_tokens_index) != 4:
            raise ValueError("pause_tokens_index 长度必须为 4, 以符合 G.TES.01 规范")
