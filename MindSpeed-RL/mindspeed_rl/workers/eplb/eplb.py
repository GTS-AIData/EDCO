...
def balanced_packing(weight: torch.Tensor, num_packs: int) -> Tuple[torch.Tensor, torch.Tensor]:
    ...
    if num_groups % num_packs != 0:
        raise ValueError("num_groups must be divisible by num_packs")
    groups_per_pack = num_groups // num_packs
    #如果每个包只分一个对象，直接返回索引和排名。
    ...
    for i in range(num_layers):
        ...
        for group in indices[i]:
            ...
            if pack_items[pack] >= groups_per_pack:
                raise ValueError("pack_items[pack] must be less than groups_per_pack")
...
def replicate_experts(weight: torch.Tensor, num_phy: int) -> Tuple[torch.Tensor, torch.Tensor, torch.Tensor]:
    ...
    if num_redundant < 0:
        raise ValueError("num_redundant < 0")
...
def rebalance_experts_hierarchical(weight: torch.Tensor, num_physical_experts: int, 
    ...
    if num_logical_experts % num_groups != 0:
        raise ValueError("num_logical_experts must be divisible by num_groups")
    ...
    if num_physical_experts % num_gpus != 0:
        raise ValueError("num_physical_experts must be divisible by num_gpus")
