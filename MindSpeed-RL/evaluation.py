...
from mindspeed_llm.tasks.evaluation.eval_impl.custom_eval import CustomEval
...
def custom_eval(eval_args, agent):
    data_path = None
    answer = None
    score_df = None
    for path in eval_args.task_data_path:
    ...
    try:
        if data_path:
            custome_eval = CustomEval(input_dir=data_path, eval_args=eval_args)
            answer, score_df = custome_eval.eval(chat=agent)
            if dist.get_rank() == 0:
                logger.info('\n{}'.format(score_df))
    except Exception as e:
        logger.info(e)
...
def main():
    ...
    if "custom_eval" in args.task:  # 自定义测评集
        a = time.time()
        custom_eval(args, LLMChat(args, model, tokenizer))
        if rank == 0:
