...
def _apply_qwen3_template(prompts):
    """适配qwen3多轮对话chat模板"""
    address_prompts = []
    for data_item in prompts:
        data_message = [{"role": "system", "content": data_item['meta_prompt'][0]}]
        data_message.extend(data_item['data'])
        address_prompts.append(data_message)
    return address_prompts
...
def _tokenize_prompts_and_batch(tokenizer, prompts, tokens_to_generate, max_generate_length, add_BOS):
    ...
    if args.hf_chat_template:
        ...
        prompts = _apply_qwen3_template(prompts)
